...
c = {'Speed': 33, 'HardDrive': 540}

with model:

    pred_post = pm.sample_posterior_predictive(trace, samples=5000, var_names=['y'], data=c)

az.hdi(pred_post['y'], hdi=0.9)

# + ex5

with model:

    pred_postt = pm.sample_posterior_predictive(trace, samples=5000, var_names=['y'])

az.hdi(pred_postt['y'], hdi=0.9)